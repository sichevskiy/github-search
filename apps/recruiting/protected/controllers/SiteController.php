...
class SiteController extends Controller
{
    ...
    {
//        $url = Yii::app()->createAbsoluteUrl('/site/username');
//        Yii::app()->user->setState('username', 'sichevskiy');
//        $this->redirect($url);
        ...
        $attributes = isset($_POST['SearchForm']) ? $_POST['SearchForm'] : Yii::app()->user->getState('search');
        if ($attributes) {
            ...
            if ($model->validate()) {
                ...
                Yii::app()->user->setState('search', $params);

                $usersData = $this->getUsers($params);

                if (!$usersData) {
                    throw new CHttpException(500, 'Could not connect to github.com');
                }

                $items = array();
                for ($page = 1; $page < $model->page; $page++) {
                    for ($i = 0; $i < 30; $i++) {
                        $items[] = null;
                    }
                }

                foreach ($usersData['items'] as $key => $item) {
                    $items[] = $item;
                }


                if (!empty($items)) {
                    $users = new CArrayDataProvider($items, array(
                            'pagination' => array(
                                'pageSize' => 30,
                            ),
                        )
                    );
                    $users->setTotalItemCount($usersData['total_count']);
                }
            }
        }

        $totalCount = isset($usersData['total_count']) ? $usersData['total_count'] : 0;
    ...
    public function actionUsername()
    {
        ...
        $this->render('username', array(
            'model' => $model,
            'commits' => $commits,
            'graphData' => $graphData,
            'candidateRate' => $this->candidateRates[$candidateRateId],
        ));
    }
    ...
    {
        $usersData = Yii::app()->gitHub->search($params);
